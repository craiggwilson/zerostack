    ...
    match parts[0] {
        ...
        "/view" => {
            ...
            if target.is_empty() {
                ...
                let names: Vec<String> = view_manager
                    .named_views()
                ...
                if names.is_empty() {
                    ...
                    for name in &names {
                        let active_marker = if view_manager.active_named().map(|id| id.as_str())
                            == Some(name.as_str())
                        {
                            " *"
                        } else {
                ...
                if !view_manager.has_view(&id) {
                    renderer.write_line(
                        &format!(
                            "no view named '{}' (use /view to list available views)",
                            target
                        ),
                        C_ERROR,
                    )?;
        ...
        "/agent" => {
            if parts.len() < 2 {
                renderer.write_line("usage: /agent <spawn|msg|view|status|stop>", C_ERROR)?;
                return Ok(());
            }
            match parts[1] {
                "spawn" => {
                    ...
                    let tool_set = match tool_preset {
                        ToolPreset::ReadOnly => ToolSet::read_only(),
                        ToolPreset::None => ToolSet::no_tools(),
                        ToolPreset::All => ToolSet::default(),
                    };
                    ...
                    {
                        Ok(id) => {
                            // Register a view for this subagent so /view <name> works.
                    ...
                    match subagent_registry.id_by_name(name) {
                        Some(id) => {
                            if let Err(e) = subagent_registry.send_message(id, message.to_string())
                            {
                                ...
                                renderer
                                    .write_line(&format!("message sent to '{}'", name), C_AGENT)?;
                            }
                        }
                        None => {
                            renderer
                                .write_line(&format!("no subagent named '{}'", name), C_ERROR)?;
                        }
                    }
                }
                "status" => {
                    ...
                    if agents.is_empty() {
                        ...
                        for snap in agents {
                            ...
                            renderer.write_line(
                                &format!(
                                    "  [{}] {} — {}{}",
                                    snap.id, snap.name, snap.status, summary
                                ),
                                C_RESULT,
                            )?;
                        }
                ...
                "stop" => {
                    ...
                    match subagent_registry.id_by_name(name) {
                        Some(id) => {
                            // If currently viewing this agent, switch back to lead first.
                            ...
                            if let Err(e) = subagent_registry.stop(id) {
                                ...
                                renderer
                                    .write_line(&format!("stopped subagent '{}'", name), C_AGENT)?;
                            }
                        }
                        None => {
                            renderer
