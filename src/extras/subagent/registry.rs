...
impl SubagentRegistry {
    ...
    pub async fn spawn(
        ...
        // Phase 2: Build per-subagent ToolContext with fresh ask channel.
        // The subagent gets its own ask_tx; the ask_rx goes to spawn_perm_relay
        // which forwards permission requests to the bus for the UI loop to handle.
        let (ask_tx, ask_rx): (ask::AskSender, ask::AskReceiver) = mpsc::channel(32);
        let subagent_ctx = ToolContext::interactive(
