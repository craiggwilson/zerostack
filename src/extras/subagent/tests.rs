...
mod tool_set_tests {
    ...
    /// Default preset includes all tools.
    #[test]
    fn default_preset_includes_all() {
        let ts = ToolSet::default();
        assert!(ts.includes("read"));
        assert!(ts.includes("write"));
        assert!(ts.includes("bash"));
        assert!(ts.includes("grep"));
        assert!(ts.includes("find_files"));
        assert!(ts.includes("list_dir"));
        assert!(ts.includes("edit"));
        assert!(ts.includes("write_todo_list"));
    }

    /// ReadOnly preset includes only read-only tools.
    #[test]
    fn read_only_preset() {
    ...
    /// None preset excludes all tools.
    #[test]
    fn none_preset_excludes_all() {
    ...
    /// Per-tool overrides take priority over preset.
    #[test]
    fn override_force_include() {
        let mut overrides = HashMap::new();
        overrides.insert("bash".to_string(), true);
        let ts = ToolSet {
            preset: ToolPreset::ReadOnly,
            overrides,
        };
        // bash is normally excluded by ReadOnly, but override forces include.
        assert!(ts.includes("bash"));
        // read is still included by preset.
        assert!(ts.includes("read"));
        // write is still excluded (no override).
        assert!(!ts.includes("write"));
    }

    /// Per-tool overrides can exclude tools from Default preset.
    #[test]
    fn override_force_exclude() {
        let mut overrides = HashMap::new();
